...
class Hangman {
    constructor() {
        this.words = ['hacker', 'developer', 'javascript', 'canvas', 'fullstack'];
        this.secretWord = '';
        this.letters = [];
        this.guessedLetter = '';
        this.errorsLeft = 10;
    ...
    checkGameOver() {
        ...
        if (gameOver) {
            setTimeout(() => canvas.gameOver(), 1500)
            return true
        } else {
            return false
            ...
            if (hangman.checkClickedLetters(e.key)) {
                if (hangman.secretWord.includes(e.key)) {
                    ...
                    for (let i = 0; i < hangman.secretWord.length; i++) {
                        if (hangman.secretWord[i] === e.key) position.push(i);
                    }
                ...
                alert('esa letra ya la ingresaste, intenta con otra.')
            }
        }
    }
};